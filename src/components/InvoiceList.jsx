...
export default function InvoiceList({ onSelectInvoice, onNewInvoice, currentInvoiceId }) {
  ...
  // localInput is used to debounce user typing before triggering the search
  ...
  // debounce typing to avoid rapid searches
  useEffect(() => {
    const t = setTimeout(async () => {
      setSearchQuery(localInput);
      await searchFirebaseInvoices(localInput);
    }, 300);
    return () => clearTimeout(t);
  }, [localInput, searchFirebaseInvoices]);
      ...
      <header className="p-3 sm:p-4 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0 mb-4">
          ...
          <button
            ...
            className="px-3 sm:px-4 py-2 bg-slate-600 text-white text-sm font-medium rounded-lg hover:bg-slate-700 transition-colors self-start sm:self-auto"
          >
            + New Invoice
          </button>
        </div>
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                ...
                setSearchQuery(localInput);
                searchFirebaseInvoices(localInput);
              }
            }}
            className="w-full px-4 py-2 pl-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-transparent text-base"
          />
            ...
            {invoices.map((invoice) => (
              <div
                ...
                className={`p-3 sm:p-4 hover:bg-slate-50 transition-colors cursor-pointer ${
                  currentInvoiceId === invoice.id ? 'bg-slate-50 border-l-4 border-l-slate-600' : ''
                }`}
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="ml-2 p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-50 rounded transition-colors flex-shrink-0"
                    >
                ...
                {/* Delete Confirmation (slate) - kept below */}
                {showDeleteConfirm === invoice.id && (
                  <div className="mt-3 p-3 bg-slate-50 border border-slate-200 rounded-lg" onClick={(e) => e.stopPropagation()}>
                    <p className="text-sm text-slate-800 mb-2">Delete this invoice?</p>
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-3 py-1 bg-slate-700 text-white text-xs font-medium rounded hover:bg-slate-800 transition-colors"
                      >
                        Delete
                      </button>
                      <button
                        ...
                        className="px-3 py-1 bg-white text-slate-700 text-xs font-medium rounded border border-slate-300 hover:bg-slate-50 transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
